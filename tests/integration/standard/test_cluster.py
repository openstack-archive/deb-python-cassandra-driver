...
class ClusterTests(unittest.TestCase):
    ...
    def test_ignored_host_up(self):
        ...
        """
        ingored_host_policy = HostFilterPolicy(RoundRobinPolicy(),
                                               predicate=lambda host: host.address not in ["127.0.0.2", "127.0.0.3"])
...
class DontPrepareOnIgnoredHostsTest(unittest.TestCase):
    ...
    def test_prepare_on_ignored_hosts(self):
        ...
        ignore_node_3_policy = HostFilterPolicy(RoundRobinPolicy(),
                                                lambda host: host.address not in ignored_addresses)

        cluster = Cluster(protocol_version=PROTOCOL_VERSION,
                          load_balancing_policy=ignore_node_3_policy)
        session = cluster.connect()
        ...
        ignored_host = next(h for h in hosts if h.address in ignored_addresses)
